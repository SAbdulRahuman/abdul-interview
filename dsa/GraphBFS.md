...
```
    ...
    adj: 0→[1,2] 1→[0,3,4] 2→[0,4] 3→[1] 4→[1,2]
...
```
    ...
    Level 1: [1, 2]       dist=1
    Level 2: [3]          dist=2
    Level 3: [4] ← found! dist=3

    Shortest path: 0 → 1 → 3 → 4 (or 0→2→3→4)
    Result: 3
```
...
```
    ...
    BFS for each island:
    Island A: start (0,0), BFS marks (0,1),(1,0),(1,1)
    Island B: start (2,2), no neighbors
    Island C: start (3,3), BFS marks (3,4)
    Result: 3
```
...
```
    Word Ladder: "hit" → "cog"
    Change one letter at a time, each step must be in wordList.

    Implicit Graph (words are vertices, edges = 1-letter diff):
    ...
    BFS levels: hit(1) → hot(2) → dot,lot(3)
                → dog,log(4) → cog(5)
    Result: 5 (length of transformation sequence)
```
...
```
    ...
    Gates at (0,2) and (3,0) are BFS sources.
    BFS expands simultaneously from both gates.
    Each INF cell gets minimum distance to any gate.
```
...
```
    ...
    Level 1: [1, 2]
    Level 2: [3, 4, 5]
    Uses size=len(queue) to track level boundaries
```
...
```
    Graph (Undirected, 6 vertices):
    adj: 0→[1,2] 1→[0,3] 2→[0,4] 3→[1,5] 4→[2,5] 5→[3,4]
    ...
    Bidirectional BFS:
    Forward (from 0):   0 → {1,2}
    Backward (from 5):  5 → {3,4}
    Forward expands:    1→3, 2→4
    → 3 is in backward visited! Meet at step 3.
    Result: 3 (0→1→3→5)
```
...
```
    ...
    01-BFS uses deque:
    Weight 0 edges → push to FRONT
    Weight 1 edges → push to BACK

    dist from 0: [0, 1, 0, 1]
    Path: 0─(0)→2─(1)→1  or  0─(0)→2─(1)→3
    Optimal: 0→2 costs 0, then 2→1 costs 1
```
...
```
    ...
    └─┬─┘ └─┬─┘ └─┬─┘
      └────┬────┘
        ...
        └───┘   │ 4 │ └───┘
                └───┘
    Peel leaves iteratively → Result: [3, 4]
```
...
```
    ...
    │     size ← len(queue)                  │
    │     process size nodes at this level   │
    ├──────────────────────────────────────┤
    │  3. Multi-Source BFS                  │
    │     queue ← [all sources]              │
    │     then standard BFS                  │
    ...
    Key: Mark visited on ENQUEUE, not dequeue!
```
